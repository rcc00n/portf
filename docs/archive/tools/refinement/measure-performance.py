...
INIT='''(() => {
 const data=window.__homepagePerf={paints:[],lcp:[],shifts:[],longTasks:[]};
...
def idle(page,cdp,seconds=6):
 ...
 after=metrics(cdp)
 observation=page.evaluate('''()=>({rootMutations:window.__rootMutations,ambientSignal:document.querySelector('.raccn-home').classList.contains('is-signal-passing'),heroVisible:document.querySelector('.raccn-home').classList.contains('is-hero-visible'),visibility:document.visibilityState,mode:document.querySelector('.raccn-home').dataset.motion,animationPlayStates:[...new Set([...document.querySelectorAll('.hp-control-plane *')].map(n=>getComputedStyle(n).animationPlayState))]})''')
...
with sync_playwright() as p:
 ...
 for name,width,height,dpr,touch,reduced in [('desktop',1440,1000,1,False,False),('mobile_touch',390,844,3,True,False),('desktop_reduced_motion',1440,1000,1,False,True)]:
  ...
  print(name,json.dumps({'FCP_ms':profile['initial']['vitals']['FCP_ms'],'LCP_ms':profile['initial']['vitals']['LCP_ms'],'CLS':profile['fullScroll']['vitals']['CLS'],'initialEncodedBytes':profile['initial']['resources']['totals']['encodedBodyBytes'],'fullEncodedBytes':profile['fullScroll']['resources']['totals']['encodedBodyBytes'],'visibleScriptSeconds':profile['idleVisible']['delta'].get('ScriptDuration'),'offscreenScriptSeconds':profile['idleOffscreen']['delta'].get('ScriptDuration'),'visibleMutations':profile['idleVisible']['rootMutations'],'offscreenMutations':profile['idleOffscreen']['rootMutations'],'errors':errors}),flush=True)
  context.close()
 browser.close()

lines=['# Homepage performance measurement','',f"Measured {report['measuredAtUTC']} against `{URL}` using production assets.",'','One fresh Chromium context per profile, HTTP cache disabled through Playwright request routing. Localhost, no CPU or network throttling. Touch/mobile is emulated on the same local Chromium host, not measured on physical mobile hardware. These are single lab samples; they are not field Core Web Vitals and do not establish an improvement against earlier measurements.','','| Profile | FCP | LCP | CLS after scroll | Initial encoded body | Full-scroll encoded body | Long tasks after scroll |','| --- | ---: | ---: | ---: | ---: | ---: | ---: |']
